...
export const ViewCertificate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    loadCertificate(certificateId).catch(() => setError('Failed to load certificate'));
  ...
  useEffect(() => {
    const fetchFile = async () => {
      ...
      setError(null);
      try {
        const res = await mockCryptoService.downloadCertificate({ ipfsCid: cid, aesKey });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">View Certificate #{certificateId || '-'}</h1>
          <p className="text-sm text-gray-600">Validate certificate details and view the document.</p>
        </div>
        {txHash && (
          <a href={getExplorerTx(txHash)} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm text-primary underline">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4">
          ...
          {cid && aesKey ? (
            ...
            <div className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">Certificate URL missing CID or key.</div>
          )}
        </div>
        <div className="space-y-3">
          <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
            {loadingFile ? <LoadingSpinner label="Decrypting and loading document..." /> : <CertificateViewer fileUrl={fileUrl} mimeType={mimeType} />}
          </div>
          {fileUrl && (
