...
app.post('/auth/verify', async (req, res) => {
  try {
    ...
    const normalized = String(address).toLowerCase();
    const result = await pool.query('SELECT nonce FROM nonces WHERE address = $1', [normalized]);
    if (result.rowCount === 0) return res.status(400).json({ error: 'Nonce not found. Request a new one.' });
